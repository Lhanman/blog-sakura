...
loginFormSubmit.click(function () {
    ...
    var phone_len = phone.val().length;
    var phone_value = phone.val();
    var pattern_1 = /^1[345789]\d{9}$/;
    if (password_len !== 0 && phone_len !== 0 && pattern_1.test(phone_value))
    {
        return true;
    }
    else
    {
        ...
        if (!pattern_1.test(phone_value) && phone_len !== 0)
        {
            phone_error.show();
        }
        if (password_len === 0)
...
modal_phone.blur(function () {
    ...
    if (modal_phone_len !== 0  )
    {
        var pattern_2 = /^1[345789]\d{9}$/;
        if (!pattern_2.test(modal_phone_value))
            {
...
change_password_btn.click(function () {
    ...
    if (change_password_btn.hasClass("yes_submit") && change_password_btn.hasClass("password_right")
                                                      && modal_phone.hasClass("phone_success"))
    {
        $.ajax({
            type:'POST',
            url:'/changePassword',
